...
export default function BuildsPage() {
  ...
  const fetchBuilds = async () => {
    try {
      ...
      // Fetch components and calculate prices for each build
      const buildsWithDetails = await Promise.all(
        allBuilds.map(async (build) => {
          try {
            ...
            const totalPrice = components.reduce((sum, bc: any) => {
              return sum + (Number(bc.components?.component_price) || 0)
            }, 0)

            // Get comments count (using build_history as proxy)
            ...
            return {
              ...
              likes: Math.floor(Math.random() * 200) + 10, // Placeholder until we add likes table
              views: Math.floor(Math.random() * 2000) + 100 // Placeholder
            }
          } catch (err) {
            console.error(`Error fetching details for build ${build.build_id}:`, err)
            return {
              ...build,
              components: [],
              totalPrice: 0,
